...
\newpage
  \section{Úvod}
    ...
    V následující kapitole se budeme věnovat konstrukci a návrhu budičů pro výkonové spínače typu IGBT pro 
    trakční aplikace. Při provozu nejen trakčních měničů, se během spínacího procesu IGBT prvku generují 
    vysoké strmosti kolektorového napětí a proudu, které vytváří rušení (EMI) a přepěťové špičky při 
    vypínání. Jelikož budič představuje elektronický obvod, pracující ve velmi těsné blízkosti výkonového 
    spínače, existuje vždy elektromagnetická vazba, přes kterou se šíří rušení a je tedy nezbytné zkoumat 
    jeho odolnost. Zpomalení spínacího procesu sice omezuje generované strmosti, ale vede ke zvyšování 
    ztrát.  Dosažení optimálního kompromisu mezi úrovní rušení a výkonovou ztrátou vede na konstrukci 
    komplexních budičů, které spínací proces výkonové součástky kontrolují ve všech jeho fázích.
   ...
   \section{Metody řízení spínacího procesu}
     ...
     \subsection{Aktivní řízení spínání (\texttt{Active gate con\-trol})}
      ...
      \begin{figure}[ht!]
        \centering
        \includegraphics[scale=0.6]{Resistive_gate_control.pdf}
      ...
      \subsubsection{Řízení spínacího procesu podle strmosti kolektorového proudu (\texttt{dI/dt control})}
        Je-li $U_{GE} = U_{d_{ON}} > U_{GE_{th}}$ je IGBT spínač ve vodivém stavu a pouze celková parazitní indukčnost 
        v obvodu zkratového proudu omezuje di/dt kolektorového proudu. Doporučovanou možností jak redukovat maximální 
        kolektorový proud $I_{C_{max}}$ a také maximální $dI_C/dt|_{max}$ je snížení hradlového napětí $U_{GE}$.
        Pro odvození závislosti strmosti kolektorového proudu na hradlovém napětí vyjděme z náhradního schématu dle 
        obr. \ref{VE:fig_eqv_cir_IGBT} a idealizovaných průběhů spínacího a vypínacího procesu výkonového spínače dle 
        obrázku *. Kolektorový proud v saturaci je možné vyjádřit vztahem
        ...
        \end{equation}
        Je-li $U_Z>U_{L_{SE}}+ U_{GE}$ pak neprochází proud skrz větev omezovače na obr. \ref{VE:fig_didt} a můžeme sestavit náhradní rovnici dle obr. **. Hradlový proud vyjádříme z rovnice (\ref{VE:eq_Ig_2}) a dosadíme do rov.\ref{VE:eq_Ic_didt2}.
   ...
   \section{Způsoby detekce nadproudu}%SHORT CIRCUIT PROTECTION
     ...
     %For IGBT application systems, it is important to construct highly reliable and low cost protection systems to prevent their destruction. One of the main protection systems is the over current protection system. In conventional over current protection methods, a collector-emitter voltage Vce monitoring method or current monitoring methods using a current transformer (CT) have been used. The Vce monitoring method detects the over current using increases of Vce under short circuit conditions. However the Vce monitoring method, for high voltage IGBTs in particular, cannot detect during the turn-on transient period because it takes a long time for Vce to decrease. Also, regarding the reliability of high voltage devices in sensing Vce, in many cases, the series connection of sensing diodes is needed, which is a serious problem. In current monitoring methods the cost of CTs is high and there is the problem of an increase in the inductance by using a CT. Recent research has reported protection methods by gate voltage control. However this method needs to be combined with a Vce detection circuit.

     Vysoká spolehlivost provozu výkonového systému, je v současnosti standardním požadavkem moderní komerční aplikace 
     ...
     %Categorization of over current conditions
       \begin{figure}[ht!]
         \centering
         \includegraphics[scale=1]{IGBT_OC_condition.pdf}
     ...
     \alpha_T\ldots$ teplotní koeficient transilu; $\Delta\vartheta_j\ldots$ oteplení nad jmenovitou teplotu okolí 25°C; $T_V\ldots$ katalogová tolerance transilu
       ...
       \begin{figure}[ht!]
         \centering
         \includegraphics[scale=0.5]{Vge_clamping.pdf}
     ...
     %A feedback to the 15V supply capacitor will clamp the gate voltage too. The effectiveness will depend on the stray inductance between gate and clamping capacitor.
